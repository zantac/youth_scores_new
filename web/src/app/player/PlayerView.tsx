...
// The player's tab (?tab=season|career|matches) is written onto the CURRENT path
// so it works on both the legacy /player?id= form and the /player/5/ path route.
function playerHref(search: string): string {
  const base = typeof window !== 'undefined' ? window.location.pathname : '/player';
...
// Player profile keyed by an explicit id prop. Rendered by the /player/[id] path
...
export default function PlayerView({ id }: { id: string }) {
  ...
  const setTab = (i: number) => {
    ...
    router.replace(playerHref(q.toString()), { scroll: false });
  };
