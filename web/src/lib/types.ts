...
export interface PlayerCareer {
  ...
  goals: number; assists: number; appearances: number; yellow_cards?: number; red_cards?: number;
  current: boolean; end_date?: string | null; status: string;
  competitions: PlayerCareerComp[];
}
export interface PlayerSeasonStats {
  season: Localized | null;
  appearances: number; goals: number; assists: number; yellow_cards: number; red_cards: number;
}
